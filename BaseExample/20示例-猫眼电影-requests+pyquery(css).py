# -*- coding:utf-8 -*-
# @Desc : 使用requests发送请求,pyquery解析响应数据,css选择器提取数据
...
def parse_movie_url(html):
    ...
    movie_urls = []
    for a in all_a:
        movie_urls.append(a.attrib["href"])
...
def parse_movie_detail(html):
    ...
    # 使用pyquery解析响应的文档信息
    doc = pq(html)
    movie_name = doc("div.movie-brief-container > h3")[0].text
    movie_type = doc("div.movie-brief-container > ul > li:nth-child(1)")[0].text
